...
const BookingCalendar = ({ carId }) => {
  ...
  useEffect(() => {
    fetch(`http://localhost:9010/cars/${id}`)
  ...
  const changeDate = (dates) => {
    if (dates) {
      ...
      console.log('Selected Dates:', dates[0].format('YYYY-MM-DD'), dates[1].format('YYYY-MM-DD'));
    } else {
      setBookings([]);
    }
  };
  ...
  const makeReservation = async () => {
    try {
      const response = await axios.post("http://localhost:9010/booking/book", {
        carId: carId,
  ...
  const fetchBookingDates = async () => {
    try {
      const response = await axios.get(`http://localhost:9010/booking/book/${carId}`, {
        withCredentials: true
      });
  ...
  const disabledDate = (current) => {
    ...
    return beforeToday || disableDates.some(({ start_date, end_date }) => {
  ...
  return (
    <div className='p-5 flex flex-col gap-3'>
      <RangePicker 
        showTime 
        onChange={changeDate} 
        value={bookings.length === 2 ? [moment(bookings[0]), moment(bookings[1])] : []} 
        disabledDate={disabledDate}
      />
      <p className='text-black font-bold'>Toplam Fiyat: {priceAccordingToDates ? priceAccordingToDates : 0} TL</p>
      <Button 
