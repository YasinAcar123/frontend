...
const CarManagement = () => {
  ...
  const fetchCars = async () => {
    try {
      const response = await fetch('http://localhost:9010/cars');
  ...
  const handleDelete = async (carId) => {
    try {
      await fetch(`http://localhost:9010/cars/${carId}`, {
        method: 'DELETE'
      });
        ...
        <Col>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group controlId="formDailyPrice">
              ...
              <Form.Control type="text" name="daily_price" value={formData.daily_price} onChange={handleChange} required />
            </Form.Group>
            ...
            <tbody>
              {cars && cars.map(car => (
                <tr key={car.car_id}>
                  ...
                  <td>
                    <Button variant="danger" onClick={() => handleDelete(car.car_id)}>Delete</Button>
                  </td>
                </tr>
              ))}
            </tbody>
...
//     if (carId) {
//       fetch(`http://localhost:9010/cars/${carId}`)
...
//     fetch(`http://localhost:9010/cars${carId ? `http://localhost:9010/${carId}` : ''}`, requestOptions)
//       .then(response => response.json())
